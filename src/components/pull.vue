...
<template>
  <div class="pull-to-refresh-app">
    <div class="content-box">
      <div class="refreshing-box" v-if="showText">
        <div>{{tipText}}</div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      _touchMove(e){
        ...
        if(this.offsetBottom<=20&&_move<0 &&_move<-100){
          this.el.style.marginTop = _move + 'px';
          this.moveDistance = touch.clientY - this.startY;
          if (_move <-50) {
            this.bottomText = '松开即可刷新'
        }}
      },
      /**
       * 下拉动作结束(松开手指)监听
      ...
      _touchEnd(e){
        ...
        let that = this;
        if (this.moveDistance > 50||this.moveDistance <-50) {
          this.tipText = '数据加载中...';
          this.bottomText = '数据加载中...';
          //调用父组件的加载数据的方法
